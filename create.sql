...
create table tb_role (id bigint not null, authority varchar(255), primary key (id));
create table tb_ticket (priority smallint check (priority between 0 and 2), status smallint check (status between 0 and 3), client_id bigint, created_at TIMESTAMP WITHOUT TIME ZONE, id bigint generated by default as identity, updated_at timestamp(6) with time zone, description varchar(255), title TEXT, primary key (id));
create table tb_ticket_category (category_id bigint not null, ticket_id bigint not null, primary key (category_id, ticket_id));
create table tb_user (ddd integer, id bigint generated by default as identity, email varchar(255) unique, name varchar(255), password varchar(255), phone varchar(255), primary key (id));
create table tb_user_role (role_id bigint not null, user_id bigint not null, primary key (role_id, user_id));
alter table if exists tb_ticket add constraint FKkmgxjhrdp5vh1hx7ey4x89rp7 foreign key (client_id) references tb_user;
alter table if exists tb_ticket_category add constraint FK8ew97ef3lwqlf7mwpm9i42gke foreign key (category_id) references tb_category;
alter table if exists tb_ticket_category add constraint FKnqi16yi7folopgb0noyphfdl3 foreign key (ticket_id) references tb_ticket;
